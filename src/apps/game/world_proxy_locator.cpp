...
#include "resource/resource_help.h"
...
void WorldProxyLocator::RegisterToLocator(int worldId, const uint64_t worldSn)
{
    std::lock_guard<std::mutex> guard(_lock);
...
void WorldProxyLocator::Remove(const int worldId, const uint64_t worldSn)
{
    std::lock_guard<std::mutex> guard(_lock);
...
bool WorldProxyLocator::IsExistDungeon(const uint64_t mspSn)
{
    std::lock_guard<std::mutex> guard(_lock);
    return _worlds.find(mspSn) != _worlds.end();
}

uint64_t WorldProxyLocator::GetWorldSnById(const int worldId)
{
    std::lock_guard<std::mutex> guard(_lock);
...
void WorldProxyLocator::HandleBroadcastCreateWorld(Packet* pPacket)
{
    ...
    const uint64_t lastWorldSn = proto.last_world_sn();
