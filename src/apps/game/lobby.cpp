...
#include "player/player_component_proto_list.h"
...
void Lobby::HandleQueryPlayerRs(Packet* pPacket)
{
    ...
    if (pLastMap != nullptr)
    {
        ...
        {
            _waitingForDungeon[pLastMap->WorldSn] = std::set<uint64_t>();
        }
...
void Lobby::EnterPublicWorld(Player* pPlayer)
{
    ...
    if (lastMapSn != (uint64_t)INVALID_ID)
    {
    ...
    {
        _waitingForWorld[pLastMap->WorldId] = std::set<uint64_t>();
    }
