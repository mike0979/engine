...
#include "resource/resource_world.h"
#include "resource/resource_manager.h"
#include "resource/resource_help.h"

#include "player/player.h"
#include "player/player_collector_component.h"
...
void WorldComponentTeleport::CreateWorldFlag(WorldProxy* pWorldProxy, int targetWorldId, TeleportObject* pObj)
{
    ...
    if (pWorldRes->IsType(ResourceWorldType::Public))
    {
        ...
        if (worldSn == static_cast<uint64_t>(INVALID_ID))
        {
            // 向appmgr申请创建地图
...
void WorldComponentTeleport::HandleBroadcastCreateWorldProxy(const int worldId, const uint64_t worldSn)
{
    const auto pResMgr = ResourceHelp::GetResourceManager();
...
void WorldComponentTeleport::BroadcastSyncPlayer(uint64_t playerSn)
{
