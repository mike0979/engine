...
Player* PlayerManagerComponent::AddPlayer(const uint64_t playerSn, uint64_t worldSn, NetIdentify* pNetIdentify)
{
...
Player* PlayerManagerComponent::GetPlayerBySn(const uint64_t playerSn)
{
    const auto iter = _players.find(playerSn);
    if (iter == _players.end())
        return nullptr;
...
void PlayerManagerComponent::RemovePlayerBySn(const uint64_t playerSn)
{
    const auto iter = _players.find(playerSn);
    if (iter == _players.end())
        return;
...
std::map<uint64_t, Player*>* PlayerManagerComponent::GetAll()
{
    return &_players;
}
