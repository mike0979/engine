...
void Player::Awake(NetIdentify* pIdentify, uint64_t playerSn, uint64_t worldSn)
{
    _account = "";
    _playerSn = playerSn;
    _player.Clear();
...
uint64_t Player::GetPlayerSN() const
{
    return _playerSn;
}
...
void Player::ParseFromStream(const uint64_t playerSn, std::stringstream* pOpStream)
{
    _playerSn = playerSn;
    _player.ParseFromIstream(pOpStream);
}

void Player::ParserFromProto(const uint64_t playerSn, const Proto::Player& proto)
{
    _playerSn = playerSn;
