...
void PlayerCollectorComponent::RemovePlayerBySocket(int socket)
{
	const auto iter = _players.find(socket);
	if (iter == _players.end())
		return;
...
void PlayerCollectorComponent::RemovePlayerBySn(uint64_t playerSn)
{
...
Player* PlayerCollectorComponent::GetPlayerBySocket(const int socket)
{
	const auto iter = _players.find(socket);
	if (iter == _players.end())
		return nullptr;
...
Player* PlayerCollectorComponent::GetPlayerBySn(uint64_t playerSn)
{
	auto iter = std::find_if(_players.begin(), _players.end(), [&playerSn](auto pair)
	{
        return pair.second->GetPlayerSN() == playerSn;
...
std::map<int, Player*>& PlayerCollectorComponent::GetAll()
{
	return _players;
}
