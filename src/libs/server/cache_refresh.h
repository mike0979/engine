...
{
public:
    //std::map<uint64_t, T*>* GetAddCache();
    //std::list<uint64_t>* GetRemoveCache();
    std::map<uint64_t, T*>* GetReaderCache();

    void AddObj(T* pObj);
    void RemoveObj(uint64_t sn);
    int Count();
...
protected:
    std::map<uint64_t, T*> _objs;
    std::map<uint64_t, T*> _adds;
    std::list<uint64_t> _removes;
};

template <class T>
inline std::map<uint64_t, T*>* CacheRefresh<T>::GetReaderCache()
{
    return &_objs;
}
...
template<class T>
inline void CacheRefresh<T>::RemoveObj(uint64_t sn)
{
