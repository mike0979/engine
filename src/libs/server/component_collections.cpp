...
IComponent* ComponentCollections::Get(const uint64_t sn)
{
    if (sn == 0)
    {
        if (!_objs.empty())
...
void ComponentCollections::Remove(uint64_t sn)
{
    _removeObjs.emplace_back(sn);
}

std::map<uint64_t, IComponent*>* ComponentCollections::GetAll()
{
    return &_objs;
}
