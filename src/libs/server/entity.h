...
public:
    ...
    template <class T, typename... TArgs>
    T* AddComponentWithSn(uint64_t sn, TArgs... args);
...
protected:
    ...
    std::map<uint64_t, IComponent*> _components;
};
...
template <class T, typename ... TArgs>
T* IEntity::AddComponentWithSn(uint64_t sn, TArgs... args)
{
...
template <class T>
uint64_t Entity<T>::GetTypeHashCode()
{
