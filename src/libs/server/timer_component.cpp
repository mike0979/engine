...
uint64_t TimerComponent::Add(const int total, const int durations, const bool immediateDo, const int immediateDoDelaySecond, TimerHandleFunction handler)
{
    // durations 执行间隔秒
    // immediateDo 是否马上执行
...
void TimerComponent::Remove(std::list<uint64_t>& timers)
{
    for (auto sn : timers)
    {
