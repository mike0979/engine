...
inline const uint64_t GetAppKey(int appType, int appId)
{
    return ((uint64_t)appType << 32) + appId;
}

inline const uint64_t GetAppKey(APP_TYPE appType, int appId)
{
    return GetAppKey((int)appType, appId);
}

inline const APP_TYPE GetTypeFromAppKey(uint64_t appKey)
{
    return (APP_TYPE)(appKey >> 32);
}

inline const int GetIdFromAppKey(uint64_t appKey)
{
