...
private:
    ...
    std::map<int, std::map<uint64_t, IMessageCallBack*>*> _callbacks;

    // 默认处理函数
    // <objsn, callback>
    std::map<uint64_t, IMessageCallBack*> _defaultCallbacks;
};
...
void MessageSystem::RegisterFunctionFilter(IEntity* obj, int msgId, std::function<T * (NetIdentify*)> getObj, std::function<void(T*, Packet*)> fun)
{
    ...
    {
        _callbacks.insert(std::make_pair(msgId, new std::map<uint64_t, IMessageCallBack*>()));
    }
