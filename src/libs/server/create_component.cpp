...
IComponent* ComponentFactoryEx(EntitySystem* pEntitySystem, std::string className, uint64_t sn, const std::tuple<TArgs...>& args, std::index_sequence<Index...>)
{
...
struct DynamicCall
{
    ...
    static IComponent* Invoke(EntitySystem* pEntitySystem, const std::string classname, uint64_t sn, std::tuple<TArgs...> t1, google::protobuf::RepeatedPtrField<::Proto::CreateComponentParam>& params)
    {
        if (params.empty())
        {
