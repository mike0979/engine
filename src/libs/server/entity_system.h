...
public:
    ...
    template <class T, typename... TArgs>
    T* AddComponentWithParent(IEntity* pParent, uint64_t sn, TArgs... args);

    template <typename... TArgs>
    IComponent* AddComponentByName(std::string className, uint64_t sn, TArgs... args);
...
private:
    ...
    std::map<uint64_t, ComponentCollections*> _objSystems;
...
template <class T, typename ... TArgs>
T* EntitySystem::AddComponentWithParent(IEntity* pParent, uint64_t sn, TArgs... args)
{
...
template<typename ...TArgs>
inline IComponent* EntitySystem::AddComponentByName(std::string className, uint64_t sn, TArgs ...args)
{
