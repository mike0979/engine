...
public:
    ...
    T* MallocObject(SystemManager* pSys, IEntity* pParent, uint64_t sn, Targs... args);
...
template <typename T>
template <typename ... Targs>
T* DynamicObjectPool<T>::MallocObject(SystemManager* pSys, IEntity* pParent, uint64_t sn, Targs... args)
{
    if (_free.empty())
    {
        if (T::IsSingle())
