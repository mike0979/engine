...
void NetworkListen::Awake(std::string ip, int port, NetworkType iType)
{
    ...
    // 快速重启地址
    int isOn = 1;
    setsockopt(_masterSocket, SOL_SOCKET, SO_REUSEADDR, (void*)&isOn, sizeof(isOn));
    ...
    {
        LOG_ERROR("::bind failed. err:" << errno);
        return;
    }
...
int NetworkListen::Accept()
{
    ...
    while (true)
    {
        const int socket = ::accept(_masterSocket, &socketClient, &socketLength);
...
uint64_t NetworkListen::GetTypeHashCode()
{
...
void NetworkListen::OnEpoll(int fd, int index)
{
    if (fd == _masterSocket)
    {
