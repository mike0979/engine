...
void Network::SetSocketOpt(int socket)
{
    ...
    if (_networkType != NetworkType::HttpConnector && _networkType != NetworkType::HttpListen)
    {
        ...
        {
            LOG_WARN("getsockopt SO_KEEPALIVE failed. err:" << errno << " socket:" << socket << " networktype:" << GetNetworkTypeName(_networkType));
        }

        setsockopt(socket, SOL_TCP, TCP_KEEPIDLE, (void*)&keepIdle, optlen);
        if (getsockopt(socket, SOL_TCP, TCP_KEEPIDLE, &keepIdle, &optlen) < 0)
        {
            LOG_WARN("getsockopt TCP_KEEPIDLE failed. err:" << errno << " socket:" << socket << " networktype:" << GetNetworkTypeName(_networkType));
        }
...
bool Network::CheckSocket(int socket)
{
    // 检查一下Socket是否有误
    int err = EBADF;
...
bool Network::CreateConnectObj(int socket, TagType tagType, TagValue tagValue, ConnectStateType iState)
{
...
void Network::Epoll()
{
    ...
    {
        int socket = _events[index].data.fd;
